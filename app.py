# JobRadar - Flask Backend (Fixed v2)
# ─────────────────────────────────────────────────────────────
# FIX 1: FRONTEND = BASE_DIR  (HTML files are in root, not /frontend/)
# FIX 2: No hardcoded API keys (use Railway Environment Variables)
# FIX 3: Background threads start at module level (Gunicorn compatible)
# FIX 4: static_files() skips api/ routes to prevent conflicts
# ─────────────────────────────────────────────────────────────
...
# ── FIX 1: HTML files live in root dir, not inside a /frontend/ subfolder ──
FRONTEND = BASE_DIR

app = Flask(__name__)
CORS(app)

# ── FIX 2: Remove hardcoded credentials — set these in Railway → Variables ──
...
def fetch_adzuna(query, country="de", n=5):
    ...
    # ── FIX 2 continued: Warn if credentials are missing ──
...
def static_files(filename):
    # ── FIX 4: Never intercept /api/ routes with the catch-all ──
...
# ── FIX 3: Module-level thread startup — works with Gunicorn, not just __main__ ──
...
def _start_background_tasks():
    ...
    if not _threads_started:
        ...
        print("[JobRadar] Background search + scheduler threads started.")
